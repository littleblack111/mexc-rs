use crate::spot::{
    v3::{enums::KlineInterval, ApiError, ApiResult, ErrorResponse},
    MexcSpotApiTrait,
};
            ...
            .map(
                |kline_value| {
                    ...
                    let open_time_ts_milliseconds = entries
                        ...
                        .ok_or(ApiError::UnableToParseResponse)?;
                    let open_time = Utc
                        .timestamp_millis_opt(open_time_ts_milliseconds)
                        .unwrap();
                    ...
                    let open = open_value
                        ...
                        .map_err(
                            |err| {
                                tracing::error!(
                                    "Unable to parse Decimal for open: {}",
                                    err
                                );
                    ...
                    let high = high_value
                        ...
                        .map_err(
                            |err| {
                                tracing::error!(
                                    "Unable to parse Decimal for high: {}",
                                    err
                                );
                                ApiError::UnableToParseResponse
                            },
                    ...
                    let low = low_value
                        ...
                        .map_err(
                            |err| {
                                tracing::error!(
                                    "Unable to parse Decimal for low: {}",
                                    err
                                );
                                ApiError::UnableToParseResponse
                            },
                    ...
                    let close = close_value
                        .parse::<Decimal>()
                        .map_err(
                            |err| {
                                tracing::error!(
                                    "Unable to parse Decimal for close: {}",
                                    err
                                );
                                ApiError::UnableToParseResponse
                            },
                    ...
                    let volume = volume_value
                        ...
                        .map_err(
                            |err| {
                                tracing::error!(
                                    "Unable to parse Decimal for volume: {}",
                                    err,
                                );
                                ApiError::UnableToParseResponse
                            },
                    ...
                    let quote_asset_volume = quote_asset_volume_value
                        .parse::<Decimal>()
                        .map_err(
                            |err| {
                                tracing::error!(
                                    "Unable to parse Decimal for quote_asset_volume: {}",
                                    err
                                );
                                ApiError::UnableToParseResponse
                            },
